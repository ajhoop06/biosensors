...
output_dir = os.path.join(base, "LIG_contacts/contact_type_results")
...
def get_seq_id():
    """
    seq_id is always passed as a positional CLI argument.
    Resolved from seq_ids.txt by contact_type_worker.sh before this
    script is called, so no SLURM env var parsing needed here.
    """
    if len(sys.argv) < 2:
        raise RuntimeError(
            "Usage: python contact_type_analysis.py <seq_id>\n"
            "seq_id is passed by contact_type_worker.sh, or provide directly for local testing."
        )
...
def main():
    ...
    summary_out = os.path.join(output_dir, f"{seq_id}_contact_summary.csv")
    if os.path.exists(summary_out):
        print(f"[{seq_id}] Output already exists, skipping: {summary_out}")
        sys.exit(0)

    traj = load_trajectory(seq_id)
    ...
    # ── Write outputs ─────────────────────────────────────────────────────
    per_frame_out = os.path.join(output_dir, f"{seq_id}_contact_perframe.csv")
    residue_out   = os.path.join(output_dir, f"{seq_id}_residue_occupancy.csv")
